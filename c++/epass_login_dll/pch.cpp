...
{
	cout <<
		"\n[FUCK ENTER:CBaseAll::BaseAllEnd:Close session and free the PKCS#11's dll]"
		<<
		endl;
	CK_RV rv = CKR_OK;
...
int epass_login(char* pin_buf, char pin_len)
{
	//printf("epass_login \n");
	//return 0;
///*
	...
	memset(m_ucPin,0,sizeof(m_ucPin));
	memcpy(m_ucPin, pin_buf,pin_len);

	printf("m_ucPin %s  \n", m_ucPin);

	rv = m_gToken->C_Login(hSession, CKU_USER, m_ucPin,strlen((char*)m_ucPin));
