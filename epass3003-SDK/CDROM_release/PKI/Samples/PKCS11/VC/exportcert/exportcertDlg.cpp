...
void CExportcertDlg::OnBtnEnum() 
{
	...
	do
	{
		...
		CK_ULONG ulSize = pAttrTemp[2].ulValueLen + 1;
		WCHAR *pStrWide = new WCHAR[ulSize];
		CK_ULONG ulConverted = MultiByteToWideChar(CP_UTF8, 0, (CHAR*)pAttrTemp[2].pValue, -1, pStrWide, ulSize);
		CHAR *pStrTemp = NULL;
		CString strvale;
		if (ulConverted > 0)
		{
			pStrTemp = new CHAR[ulConverted*2];
			WideCharToMultiByte(CP_ACP, 0, pStrWide, -1, pStrTemp, ulConverted*2, NULL, NULL);
			strvale = pStrTemp;
			delete[] pStrTemp;
		}
		delete[] pStrWide;
